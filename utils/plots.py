...
def plot_one_box(x, img, color=None, label=None, line_thickness=3):
    ...
    c_center = (int((x[0]+x[2])/2), int((x[1]+x[3])/2)) #kipp
    c_tail = (int((x[0]+x[2])), int((x[1]+x[3])/2))

    #cv2.line(img, c_center, c_tail, (67,67,67),2) #kipp
    if label:
...
def plot_two_box(x, img, color=None, label=None, line_thickness=3):
    # Plots one bounding box on image img
    tl = line_thickness or round(0.002 * (img.shape[0] + img.shape[1]) / 2) + 1  # line/font thickness
    color = color or [random.randint(0, 255) for _ in range(3)]
    c1, c2 = (int(x[0]), int(x[1])), (int(x[2]), int(x[3]))
    ...
    #cv2.line(img, c_center, c1, (67,67,67),2) #kipp
    if label:
        tf = max(tl - 1, 1)  # font thickness
        t_size = cv2.getTextSize(label, 0, fontScale=tl / 3, thickness=tf)[0]
        c2 = c1[0] + t_size[0], c1[1] - t_size[1] - 3
